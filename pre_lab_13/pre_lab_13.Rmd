...
**Answer** A line that shows relationship between Trump and the ppp loans received in certain states. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** A percentage of the vote Trump received in the statecompared to an amount per employee. The line is less dramatic than the previous one. 
```{r}
...
**Answer** Numbers that show almost no relationship between what was discussed in the previous code. 
```{r}
reveal_data %>%
...
**Answer** The t-value is approximately 4, which is much stronger than the p-value, which is around 0. That tells us that we probably won't find a statistical deviation. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**The t value is lower, but the p-value is about 1.2 so there is a higher chance of statistical anomaly. Still there is no clear evidence that Trump favored the red states when it comes to ppp loans. 
```{r}
...
**Answer**We see a dataframe that compares Clinton voting states and Trump voting states. The average amount of ppp loans per employee in blue states is smaller than red states (6,000 to 5,700). The percentage of businesses that got loans in Trump voting states is bigger than in Clinton voting states. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer**T value is low, -1, p value is about 0.2. So that most likely means there is no causation. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer**T value is -3 and much stronger than p value which is .002. That gives us an idea that this might be worth looking into. 
```{r}
