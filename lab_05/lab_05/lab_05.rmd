...
west_virginia_ppp_with_naics <- (read_csv("data/naics_codes.csv"))
...
**A1.**
126 E Burke St, 6 loans, DREW HOLDINGS LLC, BRIX27, LLC, ABRAHAM ASHTON, HUB CO-OP LLC, HUB CO-OP, LLC, RONIN PROPERTIES LLC
```{r}
zip_25401_loan <-(read_csv("data/zip-25401-loan-sample-csv.csv"))
...
**A2.**
Surely, the businesses are owned by the same "Agent". Same address can be used for the different companies, but can they actually exist if those are not online businesses? 
This is the company's website https://www.themartinsburghub.com/. Same community workspace business got loans twice. 
...
write_cvs(wv_loans_subset,"data/wv-loans-subset-cvs.csv")
```
...
berkeley_subset<- west_virginia_ppp %>%
...
berkeley_subset_clean <- read.csv("data/wv-loans-subset-cvs.csv")
berkeley_subset_clean %>%
group_by(city_original)%>%
   summarise(count_loans = n())%>%
  arrange(desc(count_loans))

martinsburg_data <- berkeley_subset_clean %>%
  filter(city_original == 'Martinsburg')
 martinsburg_data_with_naics <-martinsburg_data  %>% left_join(naics_codes)
 
 
 martinsburg_data_with_naics %>%
   group_by (title) %>%
     summarise(count_loans = n())%>%
  arrange(desc(count_loans))
...
1- Lots of restaurants in that city. 2 - chain restaurants in that city. Just by googling the city and looking at the map we can see that both are close to the truth - this small city does have plenty of restaurants and many chain ones. 
**A4.**
...
**A5.**
COTTLE CHRISTI L LLC, $ 280434.00,approved on 2021-02-17, 94 jobs retained. Known as Kitzie's.6 locations. KITZIE'S OF INWOOD is closed. 

```{r}
 martinsburg_data_with_naics %>%
  filter(title == "Full-Service Restaurants") %>%
  group_by(name,current_approval_amount,date_approved)%>%
