...
**Answer** 
Amount is <dbl> so numeric, Name is <chr>, character, id is <dbl>, numeric
...
Description: id changes from numeric to character, from <dbl> to <chr>
...
**Answer** Largest i 10000000, smallest - 6. 6 dollars for a loan is definitely suspicious and cannot be correct. The minimum mount could be not less than $1000
```{r}
...
**Answer**
 The id column is stores as <dbl>,so as a number, but we are not doing any math with that column so it's best to use <chr>
 The amount column is stored as a character, not a number
 The city column has cities spelled in different forms or misspelled
...
**Answer**
It changed 1_id to x1_id. It doesn't start with a number anymore. 
...
Description: x1_id changed to id. 
...
Meaning:
Amount column type is character and it needs to be numerical. 
...
**Answer**
5 rows. Description: fixing the character error. 
...
**Answer**
The code standardized the zip codes type to 5 digits. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
...
Answer: 2. Description:The code capitalized the first letter of all cities and got rid of the cap locks for the second column. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
...
**Answer**0. Description: Everything that starts with "Arnl"is now fixed to Arnold, so the last remaining misspeled city has been fixed by the code. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
