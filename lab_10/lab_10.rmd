...
**A1.** 54 applications have undisbursed amounts greater than $0. Illinois has the most barber shop applications and the most undisbursed applications. 
https://datawrapper.dwcdn.net/aq9yl/1/
```{r}
barber_shop_ppp<- read_csv("data/barber_shops_ppp.csv")
barber_total <- barber_shop_ppp %>%
  group_by(state) %>% 
  summarise(
    total = n()
  ) %>% arrange(desc(total))
...
barber_undisbursed <- barber_shop_ppp %>%
...
barber_final <- merge (barber_total,barber_undisbursed, by = "state" )%>%
  rename( applications=total.x, undisbursed = total.y) %>% arrange(desc(applications))

barber_10 <- barber_final %>% head (10)

write_csv(barber_10, "data/barber_10.csv")
```
...
**A2.**
https://datawrapper.dwcdn.net/G9b1k/3/

This new chart (split bars) shows the percentage of undisbursed applications out of the top 10 states that had the most barber shop applications. 
```{r}
barber_percent <- barber_10 %>%
  mutate(percentage = (undisbursed/applications)*100)%>%
  arrange(desc(percentage))

write_csv(barber_percent,"data/barber_percent.csv")
```
...
**A3.**  
Kabbage loans are more commonly found in the South East and North East part of the country,less in the South West and North West. Almost none are in the Midwest, North Caroline or California. Maybe in counties where data isn't displayed there are simply no Kabbage provided loans?

https://datawrapper.dwcdn.net/wrvwl/2/
```{r}
...
counties_with_fips <- merge(counties_final,county_fips,by = c ("match_county", "state"))

counties_percent <- counties_with_fips %>%
  mutate(percentage = (kabbage_total/total)*100)%>%
arrange(desc(percentage))

write_csv(counties_percent, "data/counties_percentage.csv")
