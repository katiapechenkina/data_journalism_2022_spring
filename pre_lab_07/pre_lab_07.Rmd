...
**Answer** US. Bureau of Statistics description of American Industry classification and naics codes. Every industry gets a specific naics sector code. 
...
**Answer**Statistics for current employment 
...
**Answer** A part of printed html code of the data
...
**Answer**4 variables with code for each 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** We see html code highlighted for the specific "Sector" section
...
**Answer**We see the data frame for naics but it has lots of information and messy 
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** We see a list of data but not complete data sets yet. 
...
**Answer** We see conversion data for certain dates.  
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** We see sector codes and industry descriptions
...
**Answer** a dataframe with 2 columns (sector and description) and 21 rows
...
**Answer** Same data frame minus one row because that row was source not an industry.  
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Data on employment, unemployment, unions, etc. 
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer** It's reading an html document. 
...
**Answer** R grabbed an html element using xpath and an id tag. 
...
**Answer**Dataset with rows and columns appears for just  one sector.  It's pretty messy. 
...
**Answer** Data set has been cleaned and gave us data for nov 2021 only - so just what we are looking for. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**Data on mining, quarrying, oil and gas extraction 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** Data on construction sector 
...
**Answer** A printed list of industries 
```{r}
...
**Answer**A list of sectors appears, but it's not in a dataframe yet. 
```{r}
...
**Answer** A printed list of industry sectors. 
```{r}
...
**Answer** Errors appears. There is an error in the name
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**Gives an example of renaming. 
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** An error, sector_name not found 
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer** A printed list of numbers 
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer** A data frame with 4 columns and 19 rows 
...
**Answer**The task loops through all industries and gives us the number of rows. 
...
**Answer** One sector data cleaned up and we get the number of employees for the given date. 
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer** We get a clean dataframe for sectors and industries, 
```{r}
# Define url of page we want to scrape
...
**Answer** URL is added to every sector. 
...
**Answer** A column with tags for html table is added to each row. 
...
**Answer** One row removed - Public Administration.
...
**Answer** We looped over each row. Each row is isolated. 
...
**Answer** Tables that have employment info identified with xpath. We see a url and xpath for each. 
```{r}
...
**Answer** We get many different dataframes with the information needed. 
...
**Answer** Cleaned up data frames with employment numbers for each sector.
...
**Answer** Same as above but now with industry sector number for each one 
...
**Answer** New data frame created and all combined info is now there. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**Mining,quarrying, oil and gas extractions statistics. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**Agricultury, forestry, fishing, hunting statistics 
...
**Answer**
 Data frame with 3 columns and 19 rows for sector number, industry, number of employees for november 2021.
```{r}
